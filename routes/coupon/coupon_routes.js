...
router.get("/getAllCoupons", async (req, res) => {
  try {
    ...
    const result = await CouponService.getAllCoupons({ name }, page, limit);
    if (result.length === 0) {
...
router.get("/user/getAllCoupons", async (req, res) => {
  try {
    ...
    const result = await CouponService.getActiveCoupons({ name }, page, limit);
    if (result.length === 0) {
      return ResponseManager.sendSuccess(res, [], 200, "No coupons found");
    }
  ...
  try {
    ...
    if (coupon) {
      ...
      ResponseManager.sendSuccess(
        ...
        [],
        200,
        "Coupon not found for status update"
      );
    }
  } catch (err) {
    ResponseManager.sendError(
...
router.get("/countCoupons", async (req, res) => {
  try {
    ...
    ResponseManager.sendSuccess(
      ...
      { count },
      200,
      "Number of coupons retrieved successfully"
    );
  } catch (err) {
    ResponseManager.sendError(
      res,
