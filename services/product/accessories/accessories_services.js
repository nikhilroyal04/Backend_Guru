...
class AccessoryService {
  ...
  async deleteAccessory(accessoryId) {
    try {
      const deletedAccessory = await AccessoryModel.findByIdAndDelete(
        accessoryId
      );
      if (!deletedAccessory) {
  ...
  async getAllAccessories(query = {}, skip = 0, limit = 20) {
    try {
      const accessories = await AccessoryModel.find(query)
        .skip(skip)
        .limit(limit);
  ...
  async getActiveAccessories(query = {}, skip = 0, limit = 20) {
    try {
      ...
      const accessories = await AccessoryModel.find(activeQuery)
        .skip(skip)
        .limit(limit);
